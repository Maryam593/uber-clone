...
const captainController = {
  getAll: async (req, res) => {
    try {
      ...
      res
        .status(200)
        .json({ success: "Data found successfully", Data: captainData });
    } catch (error) {
  ...
  LoginAsCaptain: async (req, res) => {
    try {
      ...
      const matchCredentials = await CaptainModel.findOne({ email }).select(
        "+password"
      );
      if (!matchCredentials) {
      ...
      res
        .status(200)
        .json({ Success: "Login Successfully", Data: token, matchCredentials });
    } catch (error) {
      console.log(error);
      res.status(500).json({ Error: "Internal server error" });
    }
  ...
  UpdateCaptainStatus: async (req, res) => {
    try {
      ...
      if (!id || !status)
        return res
          .status(404)
          .json({ Warning: "captain not found or couldnt updated" });
      ...
      res
        .status(200)
        .json({
          Success: "Captain status updated successfully",
          data: updatedCaptain,
        });
    } catch (error) {
      console.log(error);
    ...
    try {
      ...
      if (!email && !id) {
        return res
          ...
          .json({
            Warning:
              "Please provide either email or id to deactivate the profile.",
          });
      }
      ...
      if (!deActivateProfile) {
        return res
          .status(404)
          .json({
            Warning: "Captain profile not found or couldn't be deactivated.",
          });
      }
      console.log(res);
      res
